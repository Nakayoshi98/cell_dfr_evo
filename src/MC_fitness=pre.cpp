...
int main(int argc, const char **argv)
{
    ...
    string dir_name = "../data_random_noise" + str_delta + "/genes_" + to_string(genes) + "_MC_step_fitness=pre/num_noise_type_" + to_string(num_noise_type) + "/max=" + to_string(max_fitness);

    ifstream H_reading_file(dir_name + "/hist_f=2^-0.txt");
    while (H_reading_file.is_open())
    {
        ...
        H_reading_file.open(dir_name + "/hist_f=2^-" + to_string(renewed) + ".txt");
    }

    ifstream H_ongoing_reading_file(dir_name + "/hist_ongoing.txt");
    for (int fit = 0; fit < max_fitness + 1; fit++)
    {
        H_ongoing_reading_file >> value;
        S[fit] += value * pow(2, -renewed);
        H[fit] = value;
    }
    ...
    ifstream GRN_reading_file(dir_name + "/GRN_ongoing.txt");
    vector<vector<double>> GRN_init(genes, vector<double>(genes));
    for (int i = 0; i < genes; i++)
    {
        for (int j = 0; j < genes; j++)
        {
            if (GRN_reading_file.is_open())
            {
                GRN_reading_file >> GRN_init[i][j];
            }
            else
            {
    ...
    ifstream theta_reading_file(dir_name + "/theta_ongoing.txt");
    vector<double> theta_init(genes);
    for (int i = 0; i < genes; i++)
    {
        if (theta_reading_file.is_open())
        {
            theta_reading_file >> theta_init[i];
        }
        else
        {
    ...
    ofstream weight_writing_file;
    string hist_filename;
    ofstream hist_writing_file;
    string GRN_filename;
    ofstream GRN_writing_file;
    string theta_filename;
    ofstream theta_writing_file;
    ...
    while (true)
    {
        ...
        H[System_former.num_progenitors] += 1;
        S[System_former.num_progenitors] += f;

        if (step % 100 == 0)
        {
            if (is_flat(H))
            {
                weight_writing_file.open(dir_name + "/weight_f=2^-" + to_string(renewed) + ".txt");
                for (auto s : S)
                {
                    weight_writing_file << setprecision(15) << s << endl;
                }
                weight_writing_file.close();
                hist_writing_file.open(dir_name + "/hist_f=2^-" + to_string(renewed) + ".txt");
                for (auto h : H)
                {
                    hist_writing_file << h << endl;
                }
                hist_writing_file.close();
                f *= 0.5;
                renewed += 1;
                H = vector<long long>(max_fitness + 1, 0);
            }
        }
        if (step % 10000 == 0)
        {
            weight_writing_file.open(dir_name + "/weight_ongoing.txt");
            for (auto s : S)
            {
                weight_writing_file << setprecision(15) << s << endl;
            }
            ...
            hist_writing_file.open(dir_name + "/hist_ongoing.txt");
            for (auto h : H)
            {
                hist_writing_file << h << endl;
            }
            ...
            GRN_writing_file.open(dir_name + "/GRN_ongoing.txt");
            theta_writing_file.open(dir_name + "/theta_ongoing.txt");
            for (int i = 0; i < genes; i++)
            {
                theta_writing_file << setprecision(15) << System_former.theta[i] << " ";
                for (int j = 0; j < genes; j++)
                {
                    GRN_writing_file << setprecision(15) << System_former.GRN[i][j] << " ";
                }
                GRN_writing_file << endl;
            }
            theta_writing_file << endl;
            GRN_writing_file.close();
            theta_writing_file.close();
        }
    }
}